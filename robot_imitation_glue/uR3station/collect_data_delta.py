...
from robot_imitation_glue.collect_data_delta import collect_data, collect_data_xyz, teleoperate
...
def abs_se3_to_relative_policy_action_converter(robot_pose, abs_se3_action):
    relative_se3 = np.linalg.inv(robot_pose) @ abs_se3_action
    delta_z = relative_se3[2, 3]   # z translation difference
    return np.array([delta_z], dtype=np.float64)
...
if __name__ == "__main__":
    ...
    env = UR3eStation(with_instrumentation=True,with_spectogram_model=False)

    dataset_name = "delta_xyz_finalergfjghdfjs"
    ...
    dataset_recorder = LeRobotDatasetRecorder(
        ...
        example_action=np.zeros((3,), dtype=np.float32),
